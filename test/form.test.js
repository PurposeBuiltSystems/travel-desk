...
// --- the footer mark -----------------------------------------------------
//
// Embedded, never linked. A remote <img> would make the coordinator's mail
// client fetch from this add-in's web host every time the form is opened -
// a third host the privacy policy does not claim, blocked by default in most
// clients, and indistinguishable from a tracking pixel to anyone reviewing
// automated mail.

var L = require("../src/logo.js");
var withLogo = F.formHtml({ name: "Matthew Miller", event: "EDC-8", location: "St. Louis" },
  { orgName: "Iowa DOT", logoCid: L.CID });
var noLogo = F.formHtml({ name: "Matthew Miller", event: "EDC-8", location: "St. Louis" },
  { orgName: "Iowa DOT" });

T.check("the mark is referenced by cid, never by URL",
  withLogo.indexOf('src="cid:' + L.CID + '"') >= 0, true);
T.check("nothing in the form points at the web",
  /src="https?:/.test(withLogo), false);
T.check("it sits with the credit line, not in the header",
  withLogo.indexOf("cid:") > withLogo.indexOf("ESTIMATED TOTAL"), true);
T.check("the credit text survives alongside it",
  withLogo.indexOf("Generated by Travel Desk") >= 0, true);

// A caller that does not attach the bytes must not emit a broken image.
T.check("no cid means no img tag at all", noLogo.indexOf("<img") >= 0, false);
T.check("and the form is still complete",
  noLogo.indexOf("Generated by Travel Desk") >= 0, true);

// The bytes have to actually be a PNG, or the attachment is a broken icon.
var raw = Buffer.from(L.BYTES, "base64");
T.check("the embedded bytes decode to a PNG", raw.slice(1, 4).toString(), "PNG");
