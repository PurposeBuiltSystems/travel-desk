...
(function (root) {
  ...
  function formHtml(model, opts) {
    ...
    return [
      ...
      // The mark is optional and referenced by cid:, so a caller that does
      // not attach it still gets a valid, complete form - the tests and the
      // print preview both rely on that.
      '<p style="color:#616161;font-size:12px">' +
        (opts.logoCid
          ? '<img src="cid:' + esc(opts.logoCid) + '" width="16" height="16" alt="" ' +
            'style="vertical-align:-3px;margin-right:6px" />'
          : "") +
        "Generated by Travel Desk. The matching row was added to the shared travel planner.</p>",
      "</div>",
    ].join("\n");
  }
